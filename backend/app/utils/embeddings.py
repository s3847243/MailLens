...
async def embed_text(text: str) -> Optional[List[float]]:
    ...
    if not text:
        return None
    # OpenAI embeddings require no newlines ideally, but they handle it; keep simple
    client = _client_lazy()
...
async def embed_batch(texts: List[str]) -> List[Optional[List[float]]]:
    ...
    if not texts:
        return []
    client = _client_lazy()
    # Replace empties with "" and map back to None
...
def build_embedding_text(subject: str | None, body_text: str | None) -> str:
    ...
    # Keep compact; subject has high signal
    return f"{s}\n\n{b}".strip()


async def embed_text(_text: str) -> Optional[list[float]]:
    """M3: Replace with a real embedding call. Return a float vector or None to skip."""
    return None
